export default function createNotebook({path}){
    return function define(runtime, observer) {
      const main = runtime.module();
      const fileAttachments = new Map([[path, `/${path}`]]);
              ...
              function (pack, data, d3, width, height, DOM, color, invalidation) {
                  ...
                  const circle = leaf.append('circle')
                      ...
                      .on('mouseover', function (e, d) {
                          ...
                          tooltip.select('.first').text(d.data.buydate);
                          tooltip.select('.latest').text(d.data.lastdate);
